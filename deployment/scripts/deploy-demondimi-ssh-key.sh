...
# Alternative deployment script using SSH keys instead of password
# Use this if you have SSH key access to the server
...
echo "🚀 Starting deployment to $DOMAIN using SSH keys..."

# Function to run commands on VPS (using SSH keys)
run_on_vps() {
    ssh -o StrictHostKeyChecking=no "$VPS_USER@$VPS_HOST" "$1"
}

# Function to copy files to VPS (using SSH keys)
copy_to_vps() {
...
if ! ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "$VPS_USER@$VPS_HOST" "echo 'SSH connection successful'"; then
    ...
    echo "   Please check:"
    echo "   - You have SSH key access to $VPS_HOST"
    echo "   - Your SSH agent is running: ssh-add -l"
    echo "   - The server accepts your key"
    echo ""
    echo "   Alternative: Use manual deployment guide in MANUAL_DEPLOYMENT.md"
    exit 1
fi
...
rm -f telegram-bot.service demondimi-bot.conf .env
